...
task4(){
  ...
  if [[ $totalValidator -gt 0 ]]; then
      echo -e '\ncd /root/core-blockchain-Testnet/' >>/etc/profile
      echo -e '\nbash /root/core-blockchain-Testnet/node-start.sh --validator' >>/etc/profile
  fi

  if [[ $totalRpc -gt 0 ]]; then
      echo -e '\ncd /root/core-blockchain-Testnet/' >>/etc/profile
      echo -e '\nbash /root/core-blockchain-Testnet/node-start.sh --rpc' >>/etc/profile
  fi
